...
fn main() {
    ...
    for num in nums {
        ...
        let won_idxs = marked.iter().enumerate()
        .filter(|(_, mark)| is_win(mark))
        .map(|(idx, _)| idx)
        .collect::<Vec<_>>();

        for idx in won_idxs.iter().rev() {
            // println!("Won board {} {:?} on num {}", *idx, boards[*idx], num);

            if boards.len() == 1 {
                println!("{}", score(&boards[0], &marked[0], num));
            }
        ...
        // if boards.len() == 1 && is_win(&marked[0]) {
        //     println!("{}", score(&boards[0], &marked[0], num));
        //     break
        // }
